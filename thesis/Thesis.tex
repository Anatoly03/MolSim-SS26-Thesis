...
For example, the types \texttt{NewtonForce} and \texttt{LennardJonesForce} both implement (or inherit for the C++ codebase) the \texttt{Force} trait. For the simulation run we convert both force implementations into a dynamic \texttt{Force} object. At the cost of losing any information on the underlying type, we get a force black box capable of yielding the potential and force values for any two particles.
