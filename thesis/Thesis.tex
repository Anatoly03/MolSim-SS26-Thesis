...
The parallel benchmarks have also been computed on the supercomputer CoolMUC\cite{coolmuc-stats}, which runs on a Intel(R) Xeon(R) Platinum 8380 CPU @ 2.30GHz. This is for two reasons, to test if the in the thesis discussed artefacts with OpenMP happen on a computer with more evenly distributed processing powers contrary to toe high end PC with cores of different types. The second reason is, that the insights could be interesting or relevant to the thesis. Since we are in the Appendix, the results were not really usable in the thesis narrative, but they are still interesting insights.
...
On CoolMUC4, sequential code performs slower than on a high-end consumer PC, with the supercomputer CPU operating on a lower frequency. 
