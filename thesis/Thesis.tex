...
The first difference we observe is that there are specific thread counts, where OMP performance drops. On the machine where the benchmarks were run, there was a 1066ms drop between thread counts 8 and 9, and a 1508ms drop between thread counts 20 and 21.
...
According to the product specification of the CPU\cite{cpu-intel-core-i7-12700k}, we have 8 performance cores running at a base frequency of 3.60 GHz, each having two threads sharing one cache, and several further efficiency cores running at 2.70 GHz. When we set the thread count to 9, we either use one further efficiency core, or a thread in a already used performance core.
...
In figure~\ref{fig:benchmark-parallel-omp-modi} we see OMP tested with different modi to understand the artifacts seen previously in figure~\ref{fig:benchmark-parallel}. `Close' is defined as setting the environment variable 
