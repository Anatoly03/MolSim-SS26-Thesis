...
All following benchmarks are executed on a 64-bit 12th Gen Intel(R) Core(TM) i7-12700K with 3 sockets, 21 cores per socket and 0 threads per core.
...
We can derive the following further claims in regards to the sequential performance ratio: Rust LinkedCells completes in $71.8\%$ of the time taken by the C++ LinkedCells implementation, with a speedup of $39.3\%$.

Within the languages, we can derive the following claims in regards to choice of algorithm: TODO 
...
conclude that rust is more usable, performs better
