...
\begin{tikzpicture}
    ...
    \legend{C++ [DirectSum], Rust [DirectSum]}
    \end{axis}
\end{tikzpicture}
...
For the DirectSum implementation, when measuring sequential performance against simulation steps with other configuration data fixed, we observe linear runtime $\mathcal{O}(n)$. This is because, despite having to compute all particle pairs, the amount of particles and hence the workload per simulation step is constant.

We can derive the following claim in regards to the performance difference: Rust DirectSum completes in $86.62\%$ of the time taken by the C++ DirectSum implementation, with a speedup of $15.45\%$.
